...
function addExtension(n) {
    ...
    (v.className = "item-toggle"),
        v.setAttribute("onclick", "toggleExtension(this, '" + n.id + "');toggle(this)"),
        v.setAttribute("onmousedown", "togglePress(this, 'down')"),
        v.setAttribute("onmouseup", "togglePress(this, 'up')"),
...
async function getExtensions() {
    chrome.management.getAll(async function (n) {
        for (let t in n) n[t].isApp || addExtension({ title: n[t].name, version: n[t].version, description: n[t].description, id: n[t].id, logo: "", managed: "admin" == n[t].installType, enabled: n[t].enabled });
    }),
        setTimeout(function () {
            setIcons();
        }, 100);
